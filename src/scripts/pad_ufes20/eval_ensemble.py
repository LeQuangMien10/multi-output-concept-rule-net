"""
eval_ensemble.py - Ket hop du doan nhan cua System~1 va ICRL (System~2) cho
PAD-UFES-20. Cung 3 chien luoc voi ban Fitzpatrick
(src/scripts/fitzpatrick/eval_ensemble.py), dataset-agnostic san nen chi doi
import (load_system1/make_loaders tu train_icrl.py cua pad_ufes20) + default
--label_names.

LUU Y (khac Fitzpatrick, doc truoc khi dien giai ket qua): ban Fitzpatrick
thuong ghep ensemble voi ICRL chay --exclude_label_slot ("concept-reasoning
trung thuc hon" -- rule khong an s1_label_pred). Ban chay ICRL duy nhat cho
PAD-UFES-20 hien co (outputs/pad_ufes20_icrl) dung mac dinh (cluster_dims=
None, CO label slot), chua co ban --exclude_label_slot rieng. Van chay
ensemble duoc voi ban nay (--icrl_dir tro toi no), nhung neu muon dung dung
convention cua Fitzpatrick can chay lai train_icrl.py voi --exclude_label_slot
truoc.

3 chien luoc, tham so chon tren VAL, bao cao tren TEST:
...
Usage:
    python -m src.scripts.pad_ufes20.eval_ensemble \\
        --data_dir /kaggle/working/PAD-UFES-20_prepared \\
        --img_dir /kaggle/input/datasets/lquangmin/pad-ufes-20/images \\
        --system1_ckpt /kaggle/working/outputs/pad_ufes20_system1/best_model.pt \\
        --icrl_dir /kaggle/working/outputs/pad_ufes20_icrl
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Ensemble System1 + ICRL, PAD-UFES-20: weighted-avg / confidence-pick / gated-override.")
    ...
    p.add_argument("--icrl_dir", type=str, required=True,
                    help="Xem canh bao ve --exclude_label_slot trong docstring.")
    p.add_argument("--label_names", type=str, default=None,
                    help="Comma-separated, override cho scope khac 6-lop mac dinh.")
...
def main():
    ...
    print(f"\n--- Confusion categories: S1 alone vs Ensemble (gated-override, s1_thresh={s1_thresh}, "
          f"rule_conf_thresh={rule_conf_thresh}) ---")
